<!DOCTYPE html>
<html>
<head>
<link rel="icon" 
      type="image/png" 
      href="../images/favicon.png">
<title>Divya Enterprises-Furniture, Catrering & Other event equipments on hire, Chennai</title>
<link type="text/css" rel="stylesheet" href="../css/lightslider.css" /> 
<link href="../css/bootstrap.css" rel="stylesheet" type="text/css" media="all" />
<link href="../css/lightslider.css" rel="stylesheet" type="text/css" media="all" />
<!-- jQuery (necessary for Bootstrap's JavaScript plugins) -->
<script src="../js/jquery.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/lightgallery/1.2.6/js/lightgallery-all.min.js"></script>
<script src="../js/bootstrap.min.js"></script>
<script src="../js/lightslider.min.js"></script>
<!-- Custom Theme files -->
<!--theme-style-->
<link href="../css/style.css" rel="stylesheet" type="text/css" media="all" />	
<!--//theme-style-->
<meta name="google-site-verification" content="uANK6VKr7Pd3e1QfX0b6R2cgpoO7MaKw7Fnw4r55yjg" />
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<meta name="Description" content="Divya Enterprises in Mandaveli, Chennai. Hire furniture and equipments for weddings, corporate events, meetings, cocktail and dinner parties. We provide tables and chairs, catering equipments, crockery and themed furniture. Contact a perfect Event planner @ 9840343065"/>
<meta name="keywords" content="Wedding Product For Hire, Divya Enterprises in Mandaveli, Event Hire, Equipments Hire, Tandoor Pot, Gas Range Single Burner, Kadai, Tiffany Chairs, Bentwood Chairs, Wedding Chairs, Tolix Chairs, Ghost Chairs, Navy Stools, Tait Bar Tables, Banquet Tables, Trestle Tables, Riedel Glassware, Plumm Glassware, Bogart Cutlery, Rustic Furniture, Industrial Furniture, French Furniture, Provincial Furniture, Coastal Furniture"/>
<script type="application/x-javascript"> addEventListener("load", function() { setTimeout(hideURLbar, 0); }, false); function hideURLbar(){ window.scrollTo(0,1); } </script>
<script type="text/javascript">$(document).ready(function() {

  $Divs = $(".icon-pro");

  $Divs.click(function() {
    $Divs.removeClass("icon-light");
    $(this).addClass("icon-light");

  });
});
</script>
<script>
function show(shown,hidden,hidden1,hidden2,hidden3,hidden4,hidden5) {
  document.getElementById(shown).style.display='block';
  document.getElementById(hidden).style.display='none';
  document.getElementById(hidden1).style.display='none';
  document.getElementById(hidden2).style.display='none';
  document.getElementById(hidden3).style.display='none';
  document.getElementById(hidden4).style.display='none';
  document.getElementById(hidden5).style.display='none';
  return false;
}
</script>
<style type="text/css">
	h2{
		line-height: 200% !important;
	}
 .col-md-1{padding-left: 50px !important;}
 .thumbnail{border: 0px solid #00bce4;}
img.icon-pro{
  vertical-align: middle !important;
  display: block;
    margin-left: auto !important;
    margin-right: auto !important;
}
	img.thumbnail{
		height: 300px !important;
		margin: 0px !important;
    vertical-align: middle !important;
    background-color: #EAEAEA !important;
    cursor: pointer;
    display: block;
    margin-left: auto !important;
    margin-right: auto !important;

	}

  P{text-align: center; margin-bottom: 0px !important;}
  div.modal-body P{text-align: -moz-left !important;}

		.myDiv
{
    cursor: pointer;
    
}

</style>
<script>
  (function(i,s,o,g,r,a,m){i['GoogleAnalyticsObject']=r;i[r]=i[r]||function(){
  (i[r].q=i[r].q||[]).push(arguments)},i[r].l=1*new Date();a=s.createElement(o),
  m=s.getElementsByTagName(o)[0];a.async=1;a.src=g;m.parentNode.insertBefore(a,m)
  })(window,document,'script','https://www.google-analytics.com/analytics.js','ga');

  ga('create', 'UA-78703614-1', 'auto');
  ga('send', 'pageview');

</script>
</head>
<body>
<!-- Google Tag Manager -->
<noscript><iframe src="//www.googletagmanager.com/ns.html?id=GTM-MSC3CC"
height="0" width="0" style="display:none;visibility:hidden"></iframe></noscript>
<script>(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':
new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],
j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src=
'//www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);
})(window,document,'script','dataLayer','GTM-MSC3CC');</script>
<!-- End Google Tag Manager -->
<!--header-->
<div class="header header-top">
	<div class="container">
		<div class="logo">
		<h1><a>
			Divya Enterprises
		</a></h1>
		</div>
		<div class="top-nav">
			<span class="menu"><img src="../images/menu.png" alt=""> </span>
				<ul>
					<li><a href="../index" class="hvr-sweep-to-bottom">Home</a></li>
					<li class="active"><a href="web/Our_Products" >Our Products</a></li>
					<li><a href="web/About_us" class="hvr-sweep-to-bottom ">About Us</a></li>
					<li><a href="web/Gallery" class="hvr-sweep-to-bottom ">Gallery</a></li>
					<li><a href="web/Contact" class="hvr-sweep-to-bottom">Contact Us</a></li>
				</ul>
				<div class="clearfix"> </div>
						<!--script-->
					<script>
						$("span.menu").click(function(){
							$(".top-nav ul").slideToggle(500, function(){
							});
						});
				</script>				
		</div>
		<div class="clearfix"> </div>
	</div>
  <div class="container-fluid" style="margin: 0px;padding: 0px;height: 100%">
<P class="border-pro pull-right" data-toggle="modal" data-target="#myModal">View Full Products List</P>
<!-- Modal -->
  <div class="modal fade" id="myModal" role="dialog">
    <div class="modal-dialog" style="background-color: #eee">
    
      <!-- Modal content-->
      <div class="modal-content" styl e="background-color: #eee">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h2 class="modal-title" style="font-size: 2em;font-family: 'Cinzel-Regular';font-weight: bold;">Full Products List</h2>
        </div>
        <div class="modal-body" style="font-size: 1.3em">
          <P>Tandoor Pot</P>
          <P>Hot Plate Double Burner</P>
          <P>SS Hot Plate</P>
          <P>Gas Range Single Burner</P>
          <P>Gas Range Double Burner</P>
          <P>High Pressure Single Burner</P>
          <P>Hotel type High Pressure Single Burner</P>
          <P>Kadai (M)</P>
          <P>Kadai (L)</P>
          <P>Kadai (Extra Large)</P>
          <P>Round Tawa</P>
          <P>Tandoori Jodi</P>
          <P>Aluminium Idly Steamer</P>
          <P>SS Sweet Trays (S)</P>
          <P>SS Sweet Trays (L)</P>
          <P>SS Drum with lid - 250 litres</P>
          <P>SS Rice Strainer</P>
          <P>SS Casserole - 10 litres</P>
          <P>SS Casserole – 20 litres</P>
          <P>SS Handi (S)</P>
          <P>SS Handi (L)</P>
          <P>SS Handi (EL)</P>
          <P>SS Service Ladle</P>
          <P>SS Water Jug</P>
          <P>SS Thambalam (Big)</P>
          <P>SS Ghee Mug</P>
          <P>SS Annakuthi</P>
          <P>SS Bucket</P>
          <P>Kothu Kinnam ( 2 parts)</P>
          <P>Kothu Kinnam ( 3 parts)</P>
          <P>Kothu Kinnam ( 4 parts)</P>
          <P>Plastic Water Drum</P>
          <P>Air Circulator Fan</P>
          <P>Wet Grinder ( 5 Litres)</P>
          <P>Coconut Scrapper Machine</P>
          <P>Sauce Pan</P>
          <P>Kurupi – long size</P>
          <P>Jarni</P>
          <P>Thudupu – long size</P>
          <P>Palta kurupi</P>
          <P>Wood Handle ladle</P>
          <P>Cooler Pan</P>
          <P>SS Basin</P>
          <P>Aluminium Handi with lid (M)</P>
          <P>Aluminium Handi with lid (L)</P>
          <P>Aluminium Handi with lid (EL)</P>
          <P>Aluminium Lid</P>
          <P>Round Tawa (M)</P>
          <P>Round Tawa (L)</P>
          <P>Round Table – 4 feet</P>
          <P>Round Table – 5 feet</P>
          <P>Round Table – 6 feet</P>
          <P>Wooden Table – 6”*2” feet</P>
          <P>SS Table – 6”*2” feet</P>
          <P>Stage Tables – 6”*3” by 1.5” Height</P>
          <P>Buffer Counter with Frill – 6 feet</P>
          <P>Buffer Counter with Frill – 12 feet</P>
          <P>Canopy – 6 feet</P>
          <P>Plastic Garbage Drums</P>
          <P>Dinner Plates (Porcelain)</P>
          <P>Dessert Plates (Porcelain)</P>
          <P>Quarter Plates (Porcelain)</P>
          <P>Tea Cup/Saucer (Porcelain)</P>
          <P>Soup Cup (Porcelain)</P>
          <P>Dinner Plates (Bone China)</P>
          <P>Dessert Plates (Bone China)</P>
          <P>Quarter Plates (Bone China)</P>
          <P>Tea Cup (Bone China)</P>
          <P>Tea Cup & Saucer (Bone China)</P>
          <P>Soup Cup (Bone China)</P>
          <P>Saucers (Bone China)</P>
          <P>Juice Glass (4oZ)</P>
          <P>High Ball Glass</P>
          <P>Tea Spoons – SS</P>
          <P>AP Spoons – SS</P>
          <P>AP Forks – SS</P>
          <P>AP Knife  – SS</P>
          <P>Soup Spoons – SS</P>
          <P>Service Fork</P>
          <P>Service Spoon</P>
          <P>Rectangle Chaffing Dish (10 litres)</P>
          <P>Rectangle Chaffing Dish (10 litres) – Round type</P>
          <P>Soup Turin – Fuel type</P>
          <P>Soup Turin – Electric type</P>
          <P>Glass Bowls - Large</P>
          <P>Glass Bowls – Medium</P>
          <P>Ice cream dispensers</P>
          <P>Casserole</P>
          <P>Platters – SS – 1 feet oval</P>
          <P>Platters – SS – 2 feet oval</P>
          <P>Cruet Set</P>
          <P>Salvare (S)</P>
          <P>Salvare (M)</P>
          <P>Salvare (L)</P>
          <P>Welcome Drink Trays</P>
          <P>Ice Bucket</P>
          <P>Tea Urns – SS – 20 litres</P>
          <P>Cooler Box Food Pan</P>
          <P>Tongs </P>
          <P>Scooper</P>
          <P>Bread Basket (Bamboo)</P>
          <P>Bread Basket – SS – Large</P>
          <P>Tode Box</P>
          <P>Extra Food Container</P>
          <P>Salad Shell Bowl – Acrylics</P>
          <P>Flower Bowl with stand – Acrylic</P>
          <P>Oval Deep Platter – Acrylic</P>
          <P>Acrylic Bowls</P>
          <P>Hot Box</P>
          <P>Shamiyanah 1sqft.</P>
          <P>SS Cocktail</P>
          <P>SS 6*2 Glass Table</P>
          <P>Glass Top SS Round Table (4*4)</P>
          <P>G.N Pan</P>
          <P>SS Coffee Dabara Set</P>
          <P>Frill white</P>
          <P>Kuthuvillakku – (3 Feet)</P>
          <P>Kuthuvillakku – Big (6 Feet)</P>
          <P>SS Cattories</P>
          <P>Oil Strainer</P>
          <P>Cocktail (Mini Idly Steamer) – 400pieces</P>
          <P>Dosa Ladle SS</P>
          

        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
        </div>
      </div>
      
    </div>
  </div>
</div>
<!---->
</div>
<!--We Do-->

<!--//We Do-->

<div class="jumbotron" style="background-color: #52087D;color: white;margin: 0px;padding: 5px">

	<div class="container">
	    <div class="row" style="padding-bottom: 5px;padding-top: 5px">
      <div class="col-md-4">
	    	<h2 style="text-align: center;font-size: 2em;font-family: 'Cinzel-Regular';">PRODUCT - For Hire</h2>
        </div> 
        <div class="pull-right" style="padding-bottom: 5px;padding-top: 5px">
        </div>
	    </div>
		<div class="row" style="padding-bottom: 5px">
    <div class="col-md-2 col-xs-5 col-sm-4 myDiv" onclick="return show('Page2','Page3','Page1','Page4','Page5','Page6','con');">
        <img src="../images/icon/air.png" class="icon-pro">
        <P>Air</P>
        <P>Cooler</P>
         </div>
         <div class="col-md-2 col-xs-5 col-sm-4 myDiv" onclick="return show('Page6','Page2','Page3','Page4','Page1','Page5','con');">
        <img src="../images/icon/catering.png" class="icon-pro">
        <P>Catering</P>
        <P>Equipments</P>
      </div>
         <div class="col-md-2 col-xs-5 col-sm-4 myDiv" onclick="return show('Page3','Page2','Page4','Page1','Page5','Page6','con');">
        <img src="../images/icon/chairs3.png" class="icon-pro">
        <P>Chairs</P>
      </div>
      
      <div class="col-md-2 col-xs-5 col-sm-4 myDiv" onclick="return show('Page5','Page2','Page3','Page4','Page1','Page6','con');">
        <img src="../images/icon/fan1.png" class="icon-pro">
        <P>Fan</P>
      </div>
			<div class="col-md-2 col-xs-5 col-sm-4 myDiv" onclick="return show('Page1','Page2','Page3','Page4','Page5','Page6','con');">
				<img src="../images/icon/shamiana1.png" class="icon-pro">
        <P>Shamiana</P> 
			</div>			
			<div class="col-md-2 col-xs-5 col-sm-4 myDiv" onclick="return show('Page4','Page2','Page3','Page1','Page5','Page6','con');">
				<img src="../images/icon/tables1.png" class="icon-pro">
				<P>Tables</P>
			</div>
			
		</div>
	</div>
</div>
<div class="jumbotron" style="margin: 0px">
  <div class="container">
    <div id="con" class="row" style="display:block;">
      <P style="text-align: center;">The above listed products for Hire.. Please click each product Icon to see different types of products..</P><br/>
  
      <h3 style="text-decoration:underline;text-align: center;font-weight: bold;">Terms and conditions:</h3></br>
      <div class="col-md-7 col-md-offset-3">
        <ol style="vertical-align: middle;font-size: 1.2em">
        <li>Transport will be charged extra on actual.</li>
        <li>Order to be placed 48 hours in advance on receipt of this quotation.</li>
        <li>Breakages / Damages will be charged extra.</li>
        <li>All equipments to be washed and handed over back by the Hotel on
            the same day after the event.</li>
      </ol>
      </div>
      
      
    </div>
  <div id="Page1" class="row" style="display:none">
    <div class="container">
      <div id="myCarousel" class="carousel slide" data-ride="carousel">
     <!-- Wrapper for slides -->
        <div class="carousel-inner" role="listbox">
          <div class="item active">
            <div class="container">
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200" src="../images/equip/s1.jpg">
                <P>Shamiana Design-1</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200" src="../images/equip/s2.jpg">
                <P>Shamiana Design-2</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200" src="../images/equip/s3.jpg">
                <P>Shamiana Design-3</P>
              </div>
              
            </div>
         </div>
          <div class="item">
            <div class="container">
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/s5.jpg">
                <P>Shamiana Design-4</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/s6.jpg">
                <P>Shamiana Design-5</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/s7.jpg">
                <P>Shamiana Design-6</P>
              </div>
              
            </div>
          </div>
        </div>
     <!-- Left and right controls -->
      <a class="left carousel-control" href="#myCarousel" role="button" data-slide="prev">
      <span class="glyphicon glyphicon-chevron-left" aria-hidden="true"></span>
      <span class="sr-only">Previous</span>
      </a>
      <a class="right carousel-control" href="#myCarousel" role="button" data-slide="next">
      <span class="glyphicon glyphicon-chevron-right" aria-hidden="true"></span>
      <span class="sr-only">Next</span>
      </a>
      </div>
    </div>
  </div>
  <div id="Page2" class="row" style="display:none">
    <div class="container">
      <div id="myCarousel1" class="carousel slide" data-ride="carousel">
     <!-- Wrapper for slides -->
        <div class="carousel-inner" role="listbox">
          <div class="item active">
            <div class="container">
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/a1.jpg">
                <P>60- Litre Cooler</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/a2.jpg">
                <P>34- Litre Cooler</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/a3.jpg">
                <P>25- Litre Cooler</P>
              </div>
            </div>
         </div>
          <div class="item">
            <div class="container">
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/a1.jpg">
                <P>60- Litre Cooler</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/a2.jpg">
                <P>34- Litre Cooler</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/a3.jpg">
                <P>25- Litre Cooler</P>
              </div>
            </div>
          </div>
        </div>
     <!-- Left and right controls -->
      <a class="left carousel-control" href="#myCarousel1" role="button" data-slide="prev">
      <span class="glyphicon glyphicon-chevron-left" aria-hidden="true"></span>
      <span class="sr-only">Previous</span>
      </a>
      <a class="right carousel-control" href="#myCarousel1" role="button" data-slide="next">
      <span class="glyphicon glyphicon-chevron-right" aria-hidden="true"></span>
      <span class="sr-only">Next</span>
      </a>
      </div>
    </div>
  </div>
  <div id="Page3" class="row" style="display:none">
    <div class="container">
      <div id="myCarousel2" class="carousel slide" data-ride="carousel">
     <!-- Wrapper for slides -->
        <div class="carousel-inner" role="listbox">
          <div class="item active">
            <div class="container">
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/c1.jpg">
                <P>Groom & Bride Chair Model-1</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/c2.jpg">
                <P>Groom & Bride Chair Model-2</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/c3.jpg">
                <P>Groom & Bride Chair Model-3</P>
              </div>
              
            </div>
         </div>
          <div class="item">
            <div class="container">
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/c4.jpg">
                <P>Banquet Chair Model-1</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/c5.jpg">
                <P>Banquet Chair Model-2</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/c6.jpg">
                <P>Banquet Chair Model-3</P>
              </div>
              
            </div>
          </div>
        </div>
     <!-- Left and right controls -->
      <a class="left carousel-control" href="#myCarousel2" role="button" data-slide="prev">
      <span class="glyphicon glyphicon-chevron-left" aria-hidden="true"></span>
      <span class="sr-only">Previous</span>
      </a>
      <a class="right carousel-control" href="#myCarousel2" role="button" data-slide="next">
      <span class="glyphicon glyphicon-chevron-right" aria-hidden="true"></span>
      <span class="sr-only">Next</span>
      </a>
      </div>
    </div>
  </div>
 <div id="Page4" class="row" style="display:none">
    <div class="container">
      <div id="myCarousel3" class="carousel slide" data-ride="carousel">
     <!-- Wrapper for slides -->
        <div class="carousel-inner" role="listbox">
          <div class="item active">
            <div class="container">
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/t1.jpg">
                <P>Frill Table Model-1</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/t2.jpg">
                <P>Frill Table Model-2</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/t3.jpg">
                <P>Frill Table Model-3</P>
              </div>
              
            </div>
         </div>
          <div class="item">
            <div class="container">
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/t4.jpg">
                <P>Frill Table Model-4</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/t5.jpg">
                <P>Frill Table Model-5</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/t6.jpg">
                <P>Frill Table Model-6</P>
              </div>
              
            </div>
          </div>
        </div>
     <!-- Left and right controls -->
      <a class="left carousel-control" href="#myCarousel3" role="button" data-slide="prev">
      <span class="glyphicon glyphicon-chevron-left" aria-hidden="true"></span>
      <span class="sr-only">Previous</span>
      </a>
      <a class="right carousel-control" href="#myCarousel3" role="button" data-slide="next">
      <span class="glyphicon glyphicon-chevron-right" aria-hidden="true"></span>
      <span class="sr-only">Next</span>
      </a>
      </div>
    </div>
  </div>
  <div id="Page5" class="row" style="display:none">
    <div class="container">
      <div id="myCarousel4" class="carousel slide" data-ride="carousel">
     <!-- Wrapper for slides -->
        <div class="carousel-inner" role="listbox">
          <div class="item active">
            <div class="container">
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/f1.jpg">
                <P>Fan Type-1</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/f2.jpg">
                <P>Fan Type-2</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/f3.jpg">
                <P>Fan Type-3</P>
              </div>
              
            </div>
         </div>
          <div class="item">
            <div class="container">
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/f1.jpg">
                <P>Fan Type-1</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/f2.jpg">
                <P>Fan Type-2</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/equip/f3.jpg">
                <P>Fan Type-3</P>
              </div>
              
            </div>
          </div>
        </div>
     <!-- Left and right controls -->
      <a class="left carousel-control" href="#myCarousel4" role="button" data-slide="prev">
      <span class="glyphicon glyphicon-chevron-left" aria-hidden="true"></span>
      <span class="sr-only">Previous</span>
      </a>
      <a class="right carousel-control" href="#myCarousel4" role="button" data-slide="next">
      <span class="glyphicon glyphicon-chevron-right" aria-hidden="true"></span>
      <span class="sr-only">Next</span>
      </a>
      </div>
    </div>
  </div>
  <div id="Page6" class="row" style="display:none">
    <div class="container">
      <div id="myCarousel5" class="carousel slide" data-ride="carousel">
     <!-- Wrapper for slides -->
        <div class="carousel-inner" role="listbox">
          <div class="item active">
            <div class="container">
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/2.JPG">
                <P>Chafing Dish</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/3.JPG">
                <P>Full Food Container</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/4.JPG">
                <P>Half Food Container</P>
              </div>
            </div>
         </div>
          <div class="item">
            <div class="container">
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/5.JPG">
                <P>Cup, Knife, Fork, Spoon</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/6.JPG">
                <P>Oval SS Plater</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/7.JPG">
                <P>Sauce Pan</P>
              </div>
            </div>
          </div>
          <div class="item">
            <div class="container">
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/8.JPG">
                <P>Kothu Kinaum & Oil Stainer</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/9.JPG">
                <P>SS Drum</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/10.JPG">
                <P>SS Handi</P>
              </div>
            </div>
          </div>
          <div class="item">
            <div class="container">
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/11.JPG">
                <P>Rice Colander</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/12.JPG">
                <P>Ice Cream Dispencer</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/13.JPG">
                <P>Ice Bucket</P>
              </div>
            </div>
          </div>
          <div class="item">
            <div class="container">
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/14.JPG">
                <P>Tea Cane</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/15.JPG">
                <P>Puppet Basket</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/16.JPG">
                <P>Soup & Tea Cup</P>
              </div>
            </div>
          </div>
          <div class="item">
            <div class="container">
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/17.JPG">
                <P>Juice Glass</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/18.JPG">
                <P>Acrylic Bowl</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/19.JPG">
                <P>Acrylic Bowl</P>
              </div>
            </div>
          </div>
          <div class="item">
            <div class="container">
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/20.JPG">
                <P>Glass Bowl</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/21.JPG">
                <P>R/B Acrylic Bowl</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/22.JPG">
                <P>White Acrylic Bowl</P>
              </div>
            </div>
          </div>
          <div class="item">
            <div class="container">
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/23.JPG">
                <P>Acrylic Bowl with Stand</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/24.JPG">
                <P>Flower & Square Acrylic Bowl</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/25.JPG">
                <P>Shell Bowl</P>
              </div>
            </div>
          </div>
          <div class="item">
            <div class="container">
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/26.JPG">
                <P>Electrical Soup Pot</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/27.JPG">
                <P>Salt & Pepper</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/28.JPG">
                <P>Soccer</P>
              </div>
            </div>
          </div>
          <div class="item">
            <div class="container">
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/29.JPG">
                <P>Dust Bin</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/30.JPG">
                <P>Tode Box</P>
              </div>
              <div class="col-xs-12 col-sm-6 col-md-4">
                <img class='thumbnail' width="300" height="200"  src="../images/products/31.JPG">
                <P>Cake Tray</P>
              </div>
            </div>
          </div>
        </div>
     <!-- Left and right controls -->
      <a class="left carousel-control" href="#myCarousel5" role="button" data-slide="prev">
      <span class="glyphicon glyphicon-chevron-left" aria-hidden="true"></span>
      <span class="sr-only">Previous</span>
      </a>
      <a class="right carousel-control" href="#myCarousel5" role="button" data-slide="next">
      <span class="glyphicon glyphicon-chevron-right" aria-hidden="true"></span>
      <span class="sr-only">Next</span>
      </a>
      </div>
    </div>
  </div>
  </div>
</div>

<!--address-->

	<div class="address">
		<div class="container">
			<div class=" address-more">
				<h3>Address</h3>
				<div class="col-md-4 address-grid">
					<i class="glyphicon glyphicon-map-marker"></i>
					<div class="address1">
						<p>No 56, St Marys Road,</p><p>Mandaveli,</p>
						<p>Chennai-600 028</p>
					</div>
						<div class="clearfix"> </div>
				</div>
				<div class="col-md-3 address-grid ">
					<i class="glyphicon glyphicon-phone"></i>
						<div class="address1">
							<p>+91 9840343065</p>
							<p>+91 9840157755</p>
						</div>
					<div class="clearfix"> </div>
				</div>
				<div class="col-md-5 address-grid ">
					<i class="glyphicon glyphicon-envelope"></i>
						<div class="address1">
							<p><a href="mailto:@example.com">divyaenterprises.n@gmail.com</a></p>
						</div>
					<div class="clearfix"> </div>
				</div>
				<div class="clearfix"> </div>
			</div>
		</div>
	</div>
	<!--//address-->
	
<!--footer-->
<div class="footer">
	<div class="container">
		<div class="col-md-4 footer-top">
			<h3><a href="#">Divya Enterprises</a></h3>
		</div>
		<div class="col-md-4 footer-top1">
			<ul class="social">
						<li><a href="#"><i> </i></a></li>
            			<li><a href="#"><i class="tweet"> </i></a></li>
            			<li><a href="#"><i class="in"> </i></a></li>
            			<li><a href="#"><i class="utube"> </i></a></li>
					</ul>
		</div>
		<div class="col-md-4 footer-top2">
			<p >© 2016 Divya Enterprises. All rights reserved | Developed by <a>COAX  & TEAM</a> </p>
		</div>
		<div class="clearfix"> </div>
	</div>
</div>
</body>
</html>